...
double greedyKnapsack(double weights[], double profits[], int n, double capacity, double &time_taken, long &mem_allocated)
{
    ...
    // Sort by ascending profit-to-weight ratio
    ...
    while (i < n) {
        int idx = temp_sort[i];
        double potential_weight = total_weight + weights[idx];

        // Check if adding the next item exceeds the capacity
        if (potential_weight >= capacity) {
            // If satisfy the constraint with this item, add it and break
            total_weight = potential_weight;
            total_profit += profits[idx];
            ++i;
            break; // break to avoid adding more items and add unnecessary profit
        } else {
            // If haven't met the capacity , keep adding items
            total_weight = potential_weight;
            total_profit += profits[idx];
            ++i;
        }
    ...
    if (total_weight < capacity)
        return numeric_limits<double>::quiet_NaN();  // Infeasible case
